...
val genOrderOfCurveForInverse: unit -> Tot (lseq uint8 32)

let genOrderOfCurveForInverse () = 
...
val genOrderOfCurve: unit -> Tot (lseq uint8 32)

let genOrderOfCurve () = 
    let s = Lib.Sequence.create 32 (u8 0) in 
    let s = upd s 0 (u8 81) in 
    let s = upd s 1 (u8 37) in
    let s = upd s 2 (u8 99) in
    let s = upd s 3 (u8 252) in
    let s = upd s 4 (u8 194) in
...
let exponent_spec a = 
    let scalar = genOrderOfCurveForInverse() in 
