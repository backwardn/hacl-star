...
)
{
  ...
  Hacl_Impl_MontgomeryMultiplication_shortened_mul(primeBuffer, y, t2);
  Hacl_Impl_MontgomeryMultiplication_add8_without_carry1(t, t2, t3);
  Hacl_Impl_MontgomeryMultiplication_shift8(t3, round);
}

static void Hacl_Impl_MontgomeryMultiplication_upload_ecdsa_prime(uint64_t *p)
{
...
)
{
  ...
  uint64_t primeBuffer[4U] = { 0U };
  Hacl_Impl_MontgomeryMultiplication_upload_ecdsa_prime(primeBuffer);
  ...
  uint64_t *x = round4;
  uint64_t c = Hacl_Impl_LowLevel_sub4(x, primeBuffer, tempBuffer);
